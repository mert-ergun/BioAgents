...
def is_gpu_available_and_has_memory(required_vram_gb=4.0) -> bool:
    ...
    try:
        ...
        if free_gb < required_vram_gb:
            print(
                f"⚠️ GPU found but not enough VRAM (Free: {free_gb:.1f}GB, Required: {required_vram_gb}GB). Moving process to API..."
            )
            return False
...
def execute_model_inference(sequence, model_name="esm2_t6_8M_UR50D", required_vram_gb=2.0):
    ...
    if is_gpu_available_and_has_memory(required_vram_gb):
        ...
        if isinstance(api_key_or_signal, str) and (
            api_key_or_signal.startswith("Error:") or "[ENGAGEMENT_PENDING]" in api_key_or_signal
        ):
...
def _run_locally(_sequence, model_name):
    ...
    return {
        "status": "success",
        "source": "local_gpu",
        "result": f"{model_name} successfully ran locally.",
    }
