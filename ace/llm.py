...
def timed_llm_call(
    ...
    while True:
        try:
            ...
            is_rate_limit = any(
                k in str(e).lower() for k in ["rate limit", "429", "rate_limit_exceeded"]
            )
            is_empty_response = (
                "empty response" in str(e).lower() or "api returned none content" in str(e).lower()
            )
            ...
            if is_empty_response:
                ...
                print(f"    {prompt[:500]}...")
                print(f"📝 Full exception details: {e!r}")
                if hasattr(e, "response"):
