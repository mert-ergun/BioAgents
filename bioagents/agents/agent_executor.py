            ...
            for tool_call in response.tool_calls:
                ...
                if tool_name and tool_name in tools_dict:
                    try:
                        ...
                        tool_output = str(tool_result) if tool_result is not None else "Tool executed successfully"
                ...
                try:
                    ...
                    # Skip this tool message and continue
                    continue
        
        except Exception as e:
            logger.error(f"Agent iteration {iteration} failed: {e}", exc_info=True)
            # Return what we have so far instead of empty string
            if len(messages) > 2:
                # Try to extract any text from previous responses
