...
with open(log_file, "w", encoding="utf-8") as f:
    f.write("=" * 80 + "\n")
    f.write(f"🚀 BioAgents Interactive Demo\n")
...
log_and_print("=" * 80)
log_and_print(f"🚀 BioAgents Interactive Demo")
...
def run_query(query_text: str):
    ...
    log_and_print(f"⏰ Start time: {datetime.fromtimestamp(run_start_time).strftime('%Y-%m-%d %H:%M:%S')}\n")
                ...
                if "messages" in node_output:
                    for i, msg in enumerate(node_output["messages"]):
                        ...
                        if msg_type in ["HumanMessage", "AIMessage"]:
                            ...
                            log_and_print(f"      {content[:1000]}{'...' if len(content) > 1000 else ''}")
...
def _move_output_files(output_dir: Path, run_start_time: float):
    ...
    protected_files = {
        ...
        ".gitignore", ".env", "setup.py", "Makefile", "Dockerfile",
        "pytest.ini", "uv.lock", "poetry.lock", ".git", ".venv"
    }
    
    # Only move clearly identifiable output files (not source files)
    output_patterns = [
        "*.png", "*.jpg", "*.jpeg", "*.pdf", "*.svg",  # Images
